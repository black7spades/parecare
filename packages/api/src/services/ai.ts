...
- {"type": "add_medication", "medication_name": required, "dose": optional, "route": optional, "form": optional, "frequency": optional, "schedule_times": optional array of "HH:MM", "instructions": optional, "units_per_dose": optional number, "supply": optional number (units a full pack provides), "packs_on_hand": optional number (unopened packs), "supplier": optional (pharmacy or shop it is reordered from; reused from the account's shared supplier list, or added to it if new), "supplier_order_url": optional URL (a link to reorder it), "with_food": optional boolean, "as_needed": optional boolean, "critical": optional boolean (dangerous to miss)}
- {"type": "update_medication", "medication_name": exact name from the active list, then any of "dose", "route", "frequency", "schedule_times" (array of "HH:MM"), "instructions", "units_per_dose", "supply", "supply_remaining", "packs_on_hand", "supplier", "supplier_order_url", "with_food", "as_needed", "critical"} to change a medication, including its scheduled times
...
- {"type": "log_condition", "condition": required (the condition by name), "body": required (what happened), "entry_type": optional, one of note | symptom_change | treatment | appointment_outcome | test_result | other, "title": optional, "occurred_at": optional ISO timestamp} adds to that condition's own tracking log, kept apart from the care log. Use it for how one ongoing thing is going, e.g. an ankle sprained in March that still hurts.
...
- {"type": "add_asset", "name": required (the equipment, e.g. "ResMed AirSense 11 CPAP machine"), "category"?, "serial_number"?, "make_model"?, "price"? (number), "purchase_date"? ("YYYY-MM-DD"), "supplier"? (where bought), "warranty_expiry"? ("YYYY-MM-DD"), "condition"?, "location"?, "notes"?, "useful_life_years"? (number)} to file a piece of equipment into the asset register and link it to this profile
...
- {"type": "add_medication", "medication_name", "dose"?, "route"?, "form"?, "frequency"?, "schedule_times"? (array of "HH:MM"), "instructions"?, "units_per_dose"?, "supply"? (units a full pack provides), "packs_on_hand"? (unopened packs), "supplier"? (pharmacy or shop it is reordered from; reused from the account's shared supplier list, or added to it if new), "supplier_order_url"? (a link to reorder it), "with_food"?, "as_needed"?, "critical"?}
- {"type": "update_medication", "medication_name", then any of "dose", "route", "frequency", "schedule_times", "instructions", "units_per_dose", "supply", "supply_remaining", "packs_on_hand", "supplier", "supplier_order_url", "with_food", "as_needed", "critical"} and {"type": "stop_medication", "medication_name"}
...
- {"type": "log_condition", "condition" (by name), "body", "entry_type"? (note | symptom_change | treatment | appointment_outcome | test_result | other), "title"?, "occurred_at"?} adds to that condition's own tracking log, separate from the care log.
