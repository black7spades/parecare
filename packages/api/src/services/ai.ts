...
- {"type": "add_treatment", "name": required, "category": one of device | therapy | exercise | wound_care | diet | surgery | lifestyle | assistive_device | other, "condition_name": optional exact condition name, "price": optional number (cost of one session), "sessions_per_year": optional number (sessions expected a year, used with price for yearly health spend)} for non-medication treatments
...
- {"type": "add_medication", "medication_name", "dose"?, "route"?, "form"?, "frequency"?, "schedule_times"? (array of "HH:MM"), "instructions"?, "units_per_dose"?, "supply"? (units a full pack provides), "price"? (cost of one full pack), "packs_on_hand"? (unopened packs), "supplier"? (pharmacy or shop it is reordered from; reused from the account's shared supplier list, or added to it if new), "supplier_order_url"? (a link to reorder it), "with_food"?, "as_needed"?, "critical"?}
- {"type": "update_medication", "medication_name", then any of "dose", "route", "frequency", "schedule_times", "instructions", "units_per_dose", "supply", "supply_remaining", "price", "packs_on_hand", "supplier", "supplier_order_url", "with_food", "as_needed", "critical"} and {"type": "stop_medication", "medication_name"}
...
- {"type": "add_treatment", "name", "category" (device | therapy | exercise | wound_care | diet | surgery | lifestyle | assistive_device | other), "condition_name"?, "price"? (cost of one session), "sessions_per_year"?}
