...
export const SETTINGS_CATALOG: readonly SettingEntry[] = [
  ...
  { key: 'ai.anthropic_api_key', group: 'ai', label: 'Anthropic API key', type: 'string', secret: true, envKey: 'ANTHROPIC_API_KEY', zod: str() },
  { key: 'ai.api_key', group: 'ai', label: 'API key (OpenAI / Gemini / compatible)', type: 'string', secret: true, envKey: 'AI_API_KEY', help: 'Used for every provider except Anthropic; optional for local servers.', zod: str() },
  ...
  // OAuth sign-in
  { key: 'oauth.google_client_id', group: 'oauth', label: 'Google client ID', type: 'string', secret: false, envKey: 'GOOGLE_CLIENT_ID', zod: str() },
  { key: 'oauth.google_client_secret', group: 'oauth', label: 'Google client secret', type: 'string', secret: true, envKey: 'GOOGLE_CLIENT_SECRET', zod: str() },
  { key: 'oauth.facebook_app_id', group: 'oauth', label: 'Facebook app ID', type: 'string', secret: false, envKey: 'FACEBOOK_APP_ID', zod: str() },
  { key: 'oauth.facebook_app_secret', group: 'oauth', label: 'Facebook app secret', type: 'string', secret: true, envKey: 'FACEBOOK_APP_SECRET', zod: str() },
  ...
  // Stripe billing
  { key: 'stripe.secret_key', group: 'stripe', label: 'Stripe secret key', type: 'string', secret: true, envKey: 'STRIPE_SECRET_KEY', zod: str() },
  { key: 'stripe.webhook_secret', group: 'stripe', label: 'Stripe webhook secret', type: 'string', secret: true, envKey: 'STRIPE_WEBHOOK_SECRET', zod: str() },
  { key: 'stripe.price_family', group: 'stripe', label: 'Family plan price ID', type: 'string', secret: false, envKey: 'STRIPE_PRICE_FAMILY', zod: str() },
  { key: 'stripe.price_professional', group: 'stripe', label: 'Professional plan price ID', type: 'string', secret: false, envKey: 'STRIPE_PRICE_PROFESSIONAL', zod: str() },
] as const;
