...
function AssistantPanel({ profileId }: { profileId: string | null }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {profileId ? (
          <button
            ...
            aria-label="Upload and file a document with Pare"
            title="Upload a document, invoice or care plan for Pare to file"
            className="rounded-md border border-border px-2.5 py-2 text-muted hover:text-ink hover:bg-surface transition-colors"
          >
      ...
      {ingesting && profileId ? <IngestModal profileId={profileId} onClose={() => setIngesting(false)} /> : null}
